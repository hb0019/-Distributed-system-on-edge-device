...
import json
import os
from experimental import Cpu_limiter
...
class SystemCommandListener:
	...
	def action(self):
		while True:
			...
			if command == b'start':
				...
				p3 = Thread(target=instance_3.main, args=())
				p3.start()

			elif command == b'stop':
				p3.terminate()  # stop your own business
				print('stop my training')
	...
	def _reformat_ringkey_to_start(self, message):
		...
		template_dict = {'job_name': 'ps', 'task_index': 0, 'ps_hosts': ['192.168.0.104:22221'],
						 'worker_hosts':['192.168.0.100:22221', '192.168.0.101:22221'],
						 'batch_size': 128, 'training_epochs':120, 'learning_rate':1e-3,
						 'train_steps':1200
						 }
		port = '22221'
		worker = message['device'][:-1*message['PS_number']]  # the last one is server.
		ps_raw = [x[0] for x in message['server']]  # raw ip address
...
if __name__ == '__main__':
	ring_key = {'situation': dict(flag_of_ok_device_sum=0, system_is_running=False),
				'device': [['192.168.0.134', 0.0,0.0], ['192.168.0.100',0.0,0.0]],
				'server': [],
				'standarder':"192.168.0.193",
				'PS_number':1
				}
	instance_1 = SystemCommandListener()
	p1 = Process(target=instance_1.run, args=())
	instance_2 = auto_check.AutoCheck(ring_key=ring_key, start_flag=False)
