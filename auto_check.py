...
#	@staticmethod
	def judge_if_a_ip_can_reach(self,certain_ip):
		...
		if subprocess.call(["ping", "-c", "2", certain_ip],stdout=self.PingLogFile) == 0 or \
			...
			print("==" * 10 ,certain_ip," is success connected!", "==" * 10)
			return True
		else:
			print("==" * 10 ,"Fail to connect",certain_ip, "==" * 10)
			return False
	...
	def first_start(self):
		...
		while True:
			...
			if flag_of_ok_device_sum == Nums_of_devices and not system_is_running:
				...
				next_one_ip, _ = self._get_next_one_ip(ip_of_devices,standard_ip=_key_ring['standarder'], if_calculate=True)
	...
	def _get_next_one_ip(self, ip_of_devices, standard_ip=None, if_calculate=False):
		Nums_of_devices = len(ip_of_devices)  # nums of devices at present
		for i, c in enumerate(ip_of_devices):
			if c[0] == self.local_ip:  # found!
				if if_calculate:
					print("--" * 5,"start to test performance, it will take you about 15s !","--" * 5)
					c[1] = cst.test_calculate_time()  # calculate time 计算性能----
					c[2] = cst.test_band_width(standard_ip)	# 计算带宽----
				i += 1  # next one
				i %= Nums_of_devices
				print("**" * 5,"Token information indicate, next device is ",ip_of_devices[i][0],"**" * 5)  # del
